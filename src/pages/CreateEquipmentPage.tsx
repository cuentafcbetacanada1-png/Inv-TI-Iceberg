  ...
  return (
    <div className="space-y-6 animate-in pb-12 font-sans text-white">
      <header className="space-y-2">
         <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-[9px] font-black text-[#4e564e] hover:text-[#00ff88] transition-all uppercase tracking-[0.3em] group">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
         ...
         <div className="card-matrix p-6 border-[#1a1c1a]/60 shadow-xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-5 relative z-10">
               ...
               <InputField label="Marca de Fabricante" value={formData.marca_pc} onChange={(e: any) => setFormData({...formData, marca_pc: e.target.value})} />
               <InputField label="Modelo exacto" value={formData.modelo} onChange={(e: any) => setFormData({...formData, modelo: e.target.value})} />
               <InputField label="Dirección IP Local" value={formData.ip_local} onChange={(e: any) => setFormData({...formData, ip_local: e.target.value})} />
               <InputField icon={User} label="Usuario Responsable" value={formData.username} onChange={(e: any) => setFormData({...formData, username: e.target.value})} />
               <InputField icon={Tag} label="Número de Serie / S.N." value={formData.numero_serie} onChange={(e: any) => setFormData({...formData, numero_serie: e.target.value})} colSpan="col-span-2" />
            </div>
         </div>
         ...
         <div className="card-matrix p-6 border-[#1a1c1a]/60 shadow-xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-5 relative z-10">
               ...
               <InputField label="Arquitectura de CPU" value={formData.caracteristicas_pc} onChange={(e: any) => setFormData({...formData, caracteristicas_pc: e.target.value})} colSpan="col-span-2" />
               ...
               <InputField icon={Monitor} label="Visualizadores (Monitores detectados)" value={formData.monitores} onChange={(e: any) => setFormData({...formData, monitores: e.target.value})} colSpan="col-span-2" />
            </div>
         </div>

         {/* BOTÓN DE ACCIÓN */}
         <button type="submit" className="w-full py-4 mt-2 rounded-2xl bg-gradient-to-r from-[#00ff88] to-[#10ef87] text-black text-xs font-black italic uppercase tracking-[0.4em] hover:scale-[1.01] active:scale-95 transition-all shadow-[0_10px_30px_rgba(0,255,136,0.2)] flex items-center justify-center gap-3 col-span-1 lg:col-span-2 group">
            <Save size={18} className="group-hover:rotate-12 transition-transform" /> 
            Finalizar y Registrar en la Red
         </button>
