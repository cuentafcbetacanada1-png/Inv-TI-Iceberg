...
const SidebarItem: React.FC<{ to: string, icon: React.ElementType, label: string, active?: boolean }> = ({ to, icon: Icon, label, active }) => (
  <Link to={to} className="block group mb-3">
    <div className={cn(
      ...
      <span className="text-sm tracking-widest uppercase font-bold">{label}</span>
    </div>
  </Link>
)
...
const DashboardLayout: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-[#090a09] font-sans text-white overflow-hidden selection:bg-[#00ff88]/30">
      {/* Sidebar Emerald Matrix */}
      <aside className="w-72 bg-[#090a09] border-r border-[#0e312a] flex flex-col p-8 z-30">
        <div className="flex items-center gap-3 mb-16 px-2">
           <div className="w-10 h-10 bg-[#00ff88] rounded-xl flex items-center justify-center shadow-[0_0_20px_rgba(0,255,136,0.3)] rotate-3 group cursor-pointer hover:rotate-0 transition-transform">
              <Zap size={22} className="text-black" />
           </div>
           <div className="flex flex-col">
              <span className="text-xl font-black tracking-tighter italic uppercase text-[#00ff88]">Emerald</span>
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#4e564e] -mt-1">Inventory</span>
           </div>
        </div>
        ...
        <nav className="flex-1">
          ...
          <SidebarItem to="#" icon={Settings} label="Engine" />
        </nav>
        ...
        <div className="mt-auto space-y-6">
           <div className="p-5 rounded-3xl bg-[#0e312a]/30 border border-[#00ff88]/20 relative overflow-hidden group hover:border-[#00ff88]/40 transition-all">
              <div className="absolute -right-4 -top-4 w-16 h-16 bg-[#00ff88]/5 rounded-full group-hover:scale-150 transition-transform duration-700" />
              <div className="w-8 h-8 rounded-lg bg-[#00ff88]/20 flex items-center justify-center text-[#00ff88] mb-3">
                 <Bell size={16} />
              </div>
              <p className="text-xs font-black text-white uppercase tracking-widest mb-1">Signals Active</p>
              <p className="text-[10px] font-bold text-[#4e564e] uppercase leading-relaxed">System scan complete. All nodes stable.</p>
           </div>
           <button 
             onClick={() => signOut()}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="h-24 flex items-center justify-between px-10 z-20">
          <div className="flex items-center gap-6">
             <h2 className="text-2xl font-black tracking-tighter uppercase italic">{location.pathname === '/' ? 'Overview' : 'IT Matrix'}</h2>
             ...
             <div className="flex items-center gap-4 bg-[#121412] px-5 py-2.5 rounded-2xl border border-[#0e312a] w-80">
                ...
                <input 
                  ...
                  className="bg-transparent border-none outline-none text-xs w-full placeholder:text-[#4e564e] font-bold uppercase tracking-widest"
                />
             </div>
          </div>
          ...
          <div className="flex items-center gap-6">
             <div className="flex items-center gap-3 px-4 py-2 bg-[#121412] rounded-2xl border border-[#0e312a]">
                <div className="w-2 h-2 rounded-full bg-[#00ff88] neon-pulse shadow-[0_0_10px_#00ff88]" />
                <span className="text-[10px] font-black uppercase tracking-widest text-[#00ff88]">Live Matrix</span>
             </div>
